...
describe "assertions/new" do
  before do
    ...
    @assertion = assign(:assertion, @request.assertions.build(FactoryGirl.attributes_for(:assertion)))
  end
  ...
  it "renders new assertion form" do
    ...
    assert_select "form[action=?][method=?]", project_suite_page_request_assertions_path(@project, @suite, @page, @request), "post" do
